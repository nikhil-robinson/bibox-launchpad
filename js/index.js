const flashingBaudrateSelect = document.getElementById(
  "flashingBaudrateSelect"
);
...
const setupQRCodeContainerQS = document.getElementById(
  "setupQRCodeContainerQS"
);
...
let term = new Terminal({
  cols: utilities.getTerminalColumns(mainContainer),
  rows: 23,
  fontSize: 14,
  scrollback: 9999999,
});
...
async function buildQuickTryUI() {
  ...
  var tomlFileURL =
    window.location.origin + window.location.pathname + "config/config.toml"; // defaulting to rainmaker for now.
  ...
  xhr.onload = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      try {
        ...
        else alert("Unsupported config version used!!");
      } catch (err) {
...
document
  ...
  .addEventListener("change", utilities.handleFileSelect);
...
function buildAppLinks() {
  ...
  let defaultAppURLsHTML =
    "Note: You can download phone app from the app store and interact with your device. Scan the QRCode to access the respective apps.";
  let appURLsHTML = "";
  let setupPayloadInfo =
    "To set up the device, use a supported phone app to scan the QRCode located on the rightmost side.";
  ...
  if (setup_qrcode_payload) {
    ...
    document.getElementById("setupPayloadInfoText").innerText =
      setupPayloadInfo;
    document.getElementById("setupPayloadInfoTextQS").innerText =
      setupPayloadInfo;
  } else {
  ...
  if (appURLsHTML === defaultAppURLsHTML) {
    $("#progressMsgQS").html(
      "Firmware Image flashing is complete.<br /><br />" + appURLsHTML
    );
...
function cleanUpOldFlashHistory() {
  ...
  $("#iosAppLogoQS").html("");
  $("#progressMsgQS").html(
    "<i>This may take a short while. Check console for the progress</i>"
  );
